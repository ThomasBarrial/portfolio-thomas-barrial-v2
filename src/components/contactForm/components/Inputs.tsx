...
function Inputs({ isRequired, setName, setMail, setMessage }: IProps): JSX.Element {
  return (
    <div>
      <label className="font-bold flex flex-col mt-10">
        ...
        />
        {isRequired && <p className="text-xs mt-1 text-red-500">Ce champs est obligatoire *</p>}
      </label>
      ...
      <input className="mt-8 w-full text-ms bg-transparent border  border-pink rounded-sm p-2 focus:outline-none" type="submit" value="Send" />
    </div>
  );
}
